...
from ..core.utils import (
    arg_match, get_option, length_of, logger, Array, position_at, recycle_value
)
...
def nchar(
        ...
        keep_na: Optional[bool] = None,
        _na_len: int = 2
) -> Iterable[int]:
    """Get the size of the elements in x"""
    x, keep_na = _prepare_nchar(
        x, type, keep_na
    )
    out = [
        _nchar_scalar(
...
def nzchar(
    ...
    out = [
        NA if is_null(elem) else bool(elem)
        for elem in x
    ]
    if is_null(out).any():
        return Array(out, dtype=object)
...
def strsplit(
    ...
    out = [
        strsplit(elem, splt, fixed=fixed)
        for elem, splt in zip(x, split)
    ]
    return Array(out, dtype=object)
