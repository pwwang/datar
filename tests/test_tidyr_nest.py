...
def test_unnest_keep_empty_rows():
    df = tibble(x=f[1:3], y = [NULL, tibble(), tibble(a=1)])
    out1 = df >> unnest(f.y)
    assert nrow(out1) == 1
...
def test_unnest_rows_and_cols_of_nested_dfs_are_expanded():
    df = tibble(x = f[1:2], y = [tibble(a = 1), tibble(b = f[1:2])])
    out = df >> unnest(f.y)
...
def test_unnest_nested_lists():
    df = tibble(x=f[1:2], y=[[["a"]], [["b"]]])
    rs = unnest(df, f.y)
    assert_frame_equal(rs, tibble(x=f[1:2], y=[["a"], ["b"]]))
...
def test_can_unnest_mixture_of_named_and_unnamed():
    ...
    assert_frame_equal(out, tibble(x=["a","a"], y=f[1:2], z=f[1:2]))

def test_can_unnest_lists():
    df = tibble(x=f[1:2], y=[seq(1,3), seq(4,9)])
    out = unnest(df, f.y)
    assert_frame_equal(out, tibble(x=rep([1,2], [3,6]), y=f[1:9]))

def test_unnest_can_combine_null_with_vectors_or_dfs():
    df1 = tibble(x=f[1:2], y=[NULL, tibble(z=1)])
    out = unnest(df1, f.y)
    assert out.columns.tolist() == ['x', 'z']
    assert_iterable_equal(out.z, [1])

    df2 = tibble(x=f[1:2], y=[NULL, 1])
    out = unnest(df2, f.y)
    assert out.columns.tolist() == ['x', 'y']
    assert_iterable_equal(out.y, [1])

def test_unnest_vectors_become_columns():
    df = tibble(x=f[1:2], y=[1, [1,2]])
    out = unnest(df, f.y)
...
def test_unnest_multiple_columns_must_be_same_length():
    ...
    df = tibble(x=[[1,2]], y=[tibble(y=f[1:3])])
    with pytest.raises(ValueError, match="Incompatible lengths: 2, 3"):
        unnest(df, c(f.x, f.y))
...
def test_unnest_keeps_list_cols():
    df = tibble(x=f[1:2], y=[[3], [4]], z=[5, [6,7]])
    out = df >> unnest(f.y)
