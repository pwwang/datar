...
def test_unique(names, expect):
    assert repair_names(names, repair="unique", _base0=True) == expect
...
def test_unique_algebraic_y():
    ...
    z1 = repair_names(
        repair_names(x, repair="unique") +
        repair_names(y, repair="unique"),
        repair="unique"
    )
    z2 = repair_names(
        repair_names(x, repair="unique") + y,
        repair="unique"
    )
    z3 = repair_names(
        x + repair_names(y, repair="unique"),
        repair="unique"
    )
    z4 = repair_names(
        x + y,
...
@pytest.mark.parametrize("names,expect", [
    ...
    (repair_names(["", "_2", "_3", "__4", "___5", "____6", "_____7", "__"], repair="unique"),
     ["__0", "__1", "__2", "__3", "___5", "____6", "_____7", "__7"]),
    ([7,4,3,6,5,1,2,8],
...
def test_universal(names, expect):
    assert repair_names(names, repair="universal", _base0=True) == expect
