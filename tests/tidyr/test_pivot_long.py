...
def test_can_pivot_all_cols_to_long():
    ...
    pv2 = pivot_longer(df, f[f.x:f.y], names_transform=str.upper)
    assert_iterable_equal(pv2.name, ['X', 'X', 'Y', 'Y'])
    pv3 = pivot_longer(df, f[f.x:f.y], names_transform={'name': str.upper})
...
def test_preserves_original_keys():
    df = tibble(x=f[1:2], y=2, z=f[1:2])
    pv = pivot_longer(df, f[f.y:f.z])
...
def test_can_drop_missing_values():
    ...
    pv = pivot_longer(df, f[f.x:f.y], values_drop_na=True)
...
def test_can_override_default_output_column_type():
    df = tibble(x="x", y=1)
    pv = pivot_longer(df, f[f.x:f.y], values_transform={'value': lambda x: [x]})
    assert pv.value.tolist() == [['x'], [1]]
    pv2 = pivot_longer(df, f[f.x:f.y], values_transform=lambda x: [x])
