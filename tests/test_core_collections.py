...
from datar.core.collections import *
...
def test_collections_expand_to_indexes_with_pool():
    cs = Collection(1, 2, Collection(3, 4))
    cs = Collection(cs, pool=5, base0=False)
    assert cs == [0,1,2,3]
    cs = Collection(cs, pool=[4,3,2,1], base0=False)
    assert cs == [3,2,1,0]

    cs = Collection(-Collection(1))
    assert list(cs) == [-1]
    cs.expand(pool=10, base0=True)
    assert list(cs) == [9]

    cs = Collection(-Collection(1), pool=10, base0=True)
    assert list(cs) == [9]

def test_index0_for_base1_is_invalid():
    cs = Collection(0, pool=1, base0=False)
    assert cs == []
...
def test_unmatched_saved():
    cs = Collection(5, pool=4, base0=False)
    assert cs.unmatched == {5}

def test_strings_turn_into_integers_with_string_pool():
    cs = Collection(list("bcd"), pool=list("abc"), base0=True)
    assert cs == [1,2]
...
def test_negate_collection():
    ...
    cs = -Collection(1,2, pool=2, base0=False)
    assert cs == [0,1] # [-2, -1]

    cs = -Collection(1,2, pool=[], base0=False)
    assert cs == []
    ...
    # 'abc' mapping tto 'bcd' => [0,1]
    # Negated([0,1], pool=3, base0=True) => [-2,-1] => [1,2]
    assert list(cs) == [1,2]
...
def test_mixed_negatives_and_positives():
    ...
    #        -3      -1
    #             4 # 1-based
    cs = Collection(-Collection(1,3), 4, pool=5)
    assert list(cs) == [2, 4, 3]
...
def test_invert_collection():
    ...
    cs = Inverted(0,1,2,3,pool=5,base0=True)
    assert cs == [4]

def test_inverted_collections_elements_get_unioned_then_inverted():
    cs = Collection(~Collection(1,2), ~Collection(3,4), pool=5, base0=False)
    assert list(cs) == [4]
...
def test_slice_expands_with_pool(s):
    slc = Slice(s['b':'d'], pool=list('abcd'), base0=True)
    assert list(slc) == [1,2,3]
    # base0 doesn't affect the matching
    slc = Slice(s['b':'d'], pool=list('abcd'), base0=False)
    assert list(slc) == [1,2,3]
    slc = Slice(s['b':], pool=list('abcd'), base0=True)
    assert list(slc) == [1,2,3]
    slc = Slice(s['b':], pool=list('abcd'), base0=False)
    assert list(slc) == [1,2,3]
    slc = Slice(s['b':'d':0], pool=list('abcd'), base0=True)
    assert list(slc) == [1,2]
    slc = Slice(s['b':'d':0], pool=list('abcd'), base0=False)
    assert slc == [1,2]
    slc = Slice(s[:'c'], pool=list('abcd'), base0=True)
    assert list(slc) == [0,1,2]
    slc = Slice(s[:'c'], pool=list('abcd'), base0=False)
    assert list(slc) == [0,1,2]
    slc = Slice(s[:], pool=list('abcd'), base0=True)
    assert list(slc) == [0,1,2,3]
    slc = Slice(s[:], pool=list('abcd'), base0=False)
    assert list(slc) == [0,1,2,3]
...
def test_slice_with_int_pool(s):
    slc = Collection(s[1:3], pool=4, base0=False)
    assert list(slc) == [0,1,2]
    slc = Collection(s[1:3], pool=4, base0=True)
    assert list(slc) == [1,2]
    slc = Collection(s[:3], pool=4, base0=False)
    assert list(slc) == [0,1,2]
    slc = Collection(s[:3], pool=4, base0=True)
    assert list(slc) == [0,1,2]
    slc = Collection(s[1:], pool=4, base0=False)
    assert list(slc) == [0,1,2,3]
    slc = Collection(s[1:], pool=4, base0=True)
    assert list(slc) == [1,2,3]
    slc = Collection(s[:], pool=4, base0=False)
    assert list(slc) == [0,1,2,3]
    slc = Collection(s[:], pool=4, base0=True)
    assert list(slc) == [0,1,2,3]

def test_negated_slice(s):
    cs = Inverted(s[:2], pool=3, base0=False)
    # s[:2] => 1,2 ==index==> 0,1
    # pool=3 => [0,1,2]
    # so cs == [2]
    assert list(cs) == [2]
    cs = Inverted(s[:2], pool=3, base0=True)
    # s[:2] => 0,1 ==index==> 0,1
    # pool=3 => [0,1,2]
    # so cs == [2]
    assert list(cs) == [2]
...
def test_mixed_collections(s):
    cs = Collection(1,2,s[3:4],-Collection(1,2))
    assert cs == [1,2,3,4,-1,-2]

    cs = Collection(3, Collection(5), s[6:7], -Collection(1), pool=10, base0=True)
...
def test_intersect():
    ...
    ins = Intersect(left, right).expand()
    assert list(ins) == [1,2] # returns 0-based indexes

    # mixed str and index get correctly intersected
    left = Collection(1,2,'c', pool=list('abcd'))
    right = Collection('b',3,4, pool=list('abcd'))
    ins = Intersect(left, right).expand()
    assert list(ins) == [1,2]
