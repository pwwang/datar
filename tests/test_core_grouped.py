...
def test_instance():
    ...
    gf = DataFrameGroupBy({'a': [1,2,3]}, _group_vars=['a'], _group_drop=False)
...
def test_apply():
    ...
    def n(subdf, skip=None):
        ...
        return DataFrame({"n": [len(
            subdf.attrs['_group_data'].loc[subdf.attrs['_group_index'], '_rows']
        )]})

    gf = DataFrameGroupBy(df, _group_vars=['a'])
    out = gf.datar_apply(n)
    exp = DataFrame({
        'a': df.a,
        'n': [1,1]
    })
...
def test_apply_returns_none():
    df = DataFrame({'a': []})
    gf = DataFrameGroupBy(df, _group_vars=['a'])
    out = gf.datar_apply(lambda df: None)
    assert out is None
...
def test_rowwise():
    ...
    with pytest.raises(ValueError):
        rf.datar_apply(lambda df: None, _spike_groupdata=DataFrame())
