...
def test_expand_completes_all_values():
    ...
    assert_frame_equal(out, tibble(x=[1,1,2,2,], y=[1,2,1,2]))
...
def test_expand_respects_groups():
    ...
    assert_frame_equal(out.data.values[1].reset_index(drop=True), tibble(b=1, c=1))
...
def test_expand_grid():
    ...
    assert_frame_equal(out, tibble(df=tibble(x=[1,1,1,2,2,2],
                                             y=[2,2,2,1,1,1]),
                                   z=[1,2,3,1,2,3]))
    ...
    assert_frame_equal(
        out,
        tibble(x1=tibble(a=[1,1,2,2], b=[3,3,4,4]),
