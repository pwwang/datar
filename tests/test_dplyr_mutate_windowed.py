...
def test_cum_sum_min_max_works():
    ...
    res = mutate(
        ...
        cmaxx=cummax(f.x), cmaxy=cummax(f.y)
    )
    ...
    res = mutate(group_by(df, f.g),
        csumx = cumsum(f.x), csumy = cumsum(f.y),
        cminx = cummin(f.x), cminy = cummin(f.y),
        cmaxx = cummax(f.x), cmaxy = cummax(f.y)
    )
    assert_iterable_equal(res.csumx, c(list(cumsum(df.x[:5])), list(cumsum(df.x[5:]))))
    assert_iterable_equal(res.csumy, c(list(cumsum(df.x[:5])), list(cumsum(df.x[5:]))))
    assert_iterable_equal(res.cminx, c(list(cummin(df.x[:5])), list(cummin(df.x[5:]))))
    assert_iterable_equal(res.cminy, c(list(cummin(df.x[:5])), list(cummin(df.x[5:]))))
