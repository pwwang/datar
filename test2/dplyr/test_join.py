...
def test_nest_join_returns_list_of_dfs():
    ...
    assert out.columns.tolist() == ["x", "y", "_y_joined"]
    df2list = out._y_joined.tolist()
...
def test_nest_join_handles_multiple_matches_in_x():
    ...
    assert out._y_joined.values[0].equals(out._y_joined.values[1])
...
def test_y_keys_dropped_by_default():
    ...
    out = nest_join(df1, df2, by="x")
    assert out.columns.tolist() == ["x", "y", "_y_joined"]
    assert out._y_joined.values[0].columns.tolist() == ["z"]

    out = nest_join(df1, df2, by="x", keep=True)
    assert out._y_joined.values[0].columns.tolist() == ["x", "z"]
...
def test_nest_join_by_multiple():
    ...
    assert out._y_joined.values[0].equals(tibble(z=5))
    assert out._y_joined.values[1].equals(tibble(z=6, _index=1))
